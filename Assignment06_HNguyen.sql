...
WHERE i.EmployeeID = (
    SELECT MIN(i2.EmployeeID)
    FROM dbo.Inventories AS i2
    WHERE i2.InventoryDate = i.InventoryDate
)
ORDER BY
    i.InventoryDate;
GO
...
JOIN dbo.vInventories i ON p.ProductID = i.ProductID
JOIN dbo.vEmployees e ON i.EmployeeID = e.EmployeeID
LEFT JOIN dbo.vEmployees m ON e.ManagerID = m.EmployeeID;  -- Left join in case some employees have no manager
GO
